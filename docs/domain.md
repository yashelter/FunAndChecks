...
| `Student` | + `Color`, `IsActive`, `CreatedAt`, `GroupId`/`Group`, коллекции `Submissions`, `QueueEntries`, `Grades` | Студент — сдаёт задания, стоит в очередях |
...
- `Student.Color` и `Admin.Color` — независимые hex-цвета (`#RRGGBB`). Цвет студента (если задан)
  заливает фон ячейки с его ФИО в таблице результатов и в очереди (null — без заливки); назначается
  **только админом**. Цвет/буква админа используются в ячейках задач таблицы результатов.
- `Student.IsActive` — подтвердил ли студент почту. Неактивные не показываются в рейтинге/очередях и
  удаляются фоновой очисткой по истечении окна подтверждения (см. [security.md](security.md)).
...
В итоговый балл (`TotalPoints`) входят только принятые задачи (`MaxPoints`); баллы за оценочные колонки отображаются отдельными столбцами и в сумму **не входят**.

...
- `Student.Color` и `Admin.Color` — независимые hex-цвета (`#RRGGBB`). Цвет студента по умолчанию
  ни на что не влияет; цвет/буква админа используются в ячейках сводной таблицы результатов.
...
| `GradeComponent` | `Id`, `Name`, `MaxPoints`, `SubjectId` | Оценочная колонка предмета |
...
| Сущность | Поля | Связи |
|----------|------|-------|
| `QueueEvent` | `Id`, `Name`, `EventDateTime`, `SubjectId` | Событие сдачи по предмету; имеет участников |

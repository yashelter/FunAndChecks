...
## Изменения против предыдущей версии (breaking)

- `GET /api/students/{studentId}/subjects/{subjectId}/tasks`, `GET /api/subjects/{subjectId}/tasks`
  и `GET /api/subjects/{subjectId}/grade-components` теперь требуют роль админа (ранее были публичными).
- `POST /api/auth/register` возвращает нейтральный `201` без тела (ранее — `{id}`); повторная
  регистрация на подтверждённый email больше не возвращает `409`.
- Методы админа учитывают ограничения, выданные супер-админом: недоступные предметы/группы
  дают `403` (ранее данные отдавались без проверки).
- `GET /api/queues` включает события, истёкшие не более чем на 24 часа (ранее — 48).
- Формат `FullName` — «Фамилия Имя» (ранее «Имя Фамилия»); влияет на отображение и сортировку.

## Auth — `/api/auth`

Все эндпоинты под rate-limit политикой `auth` (20 запросов/мин с IP). Дополнительно — не чаще
**одного письма в минуту на email** (подтверждение/сброс); превышение → `429` с понятным сообщением.

| Метод | Путь | Доступ | Описание |
|-------|------|--------|----------|
| POST | `/register` | 🌐 | Регистрация студента (неактивен до подтверждения); нейтральный `201` не раскрывает, существовал ли аккаунт. Повторная регистрация на тот же неподтверждённый email заменяет незавершённую запись |
...
| PUT | `/preferences/culture` | 🔒 | Совместимый alias сохранения `en-US`/`ru-RU`; основной путь — `/api/account/preferences/culture` |
...
| PUT | `/{studentId}/account` | ⭐ | Редактировать профиль и аккаунт студента (ФИО, группа, email, пароль) → `204`; ошибки: `400` валидация, `404`, `409 account.email_taken` |
...
| Метод | Путь | Доступ | Описание |
|-------|------|--------|----------|
| GET | `/` | 🌐 | Активные события (не истёкшие более чем на 24 часа) |
| GET | `/all` | 🌐 | Все события |
...
| Ситуация | HTTP |
|----------|------|
| Ошибка валидации | 400 (с разбивкой ошибок по полям) |
| Требуется аутентификация | 401 |
| Нет доступа | 403 |
| Ресурс не найден | 404 |
| Конфликт состояния/целостности | 409 |
| Превышен лимит | 429 |
| Безопасная внутренняя ошибка | 500 |

Rate-limit отдаёт `429`. Подробнее об авторизации — [docs/security.md](security.md).

Все API-ошибки, включая `401`, `403`, `404`, `405` и `429` из middleware, используют один
обратно совместимый контракт. Помимо `detail` и `errors`, ответ содержит стабильный `code`,
именованные скалярные `args` и, для ошибок полей, `validationCodes`. UI переводит сообщения по
этим кодам; неизвестный клиент может продолжать использовать `detail`.

Поддерживаемые культуры — `en-US` и `ru-RU`. Клиент передаёт выбор в `Accept-Language`, а
авторизованный пользователь сохраняет его через `PUT /api/account/preferences/culture`
(`PUT /api/me/preferences/culture` оставлен как совместимый alias).

...
> **Авто-очереди**: при создании можно передать `AutoFillGroupId` — в очередь сразу добавляются все
> студенты группы, а самостоятельная запись отключается (`AllowSelfJoin=false`). Попытка `join` в такую
> очередь → `403`.
